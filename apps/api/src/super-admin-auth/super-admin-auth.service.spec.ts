...
// Only covers deleteAccount (feat/171) -- login/refresh/logout/getProfile
// have no unit spec of their own yet (their behavior is exercised by
// super-admin-auth.e2e-spec.ts instead), so this file stays scoped to the
// new method rather than retrofitting coverage for pre-existing ones.
type PrismaMock = {
  superAdminAccount: {
...
describe('SuperAdminAuthService.deleteAccount', () => {
  ...
  it('rejects an incorrect password without deleting anything', async () => {
    ...
    await expect(
      service.deleteAccount('super-admin-1', { password: 'wrong-password' }),
  ...
  it('rejects deleting the last remaining super-admin account', async () => {
    ...
    await expect(
      service.deleteAccount('super-admin-1', {
